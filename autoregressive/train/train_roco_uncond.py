#!/usr/bin/env python3
"""Train a text-free LlamaGen prior over ROCOv2 base-token maps."""
...
from pafr_vq.rocov2 import RocoImageListDataset
...
def _setup_distributed():
    ...
    if not dist.is_initialized():
        dist.init_process_group("nccl", rank=rank, world_size=world, device_id=torch.device("cuda", local_rank))
    return rank, world, local_rank, torch.device("cuda", local_rank)


def _logger(output_dir, rank):
    logger = logging.getLogger(f"roco_uncond.rank{rank}")
    ...
    file_handler = logging.FileHandler(Path(output_dir) / "log.txt", encoding="utf-8")
...
def _update_link(checkpoint, link):
    checkpoint = Path(checkpoint); link = Path(link); temporary = link.with_name("." + link.name + ".tmp")
    if temporary.exists() or temporary.is_symlink(): temporary.unlink()
    temporary.symlink_to(checkpoint.name); os.replace(temporary, link)


def _save(model, optimizer, scaler, args, checkpoint_dir, epoch, step, best, metrics, is_best):
    path = Path(checkpoint_dir) / f"{step:07d}.pt"
    payload = {
        "model": model.module.state_dict(), "optimizer": optimizer.state_dict(),
        "scaler": scaler.state_dict(), "epoch": epoch, "steps": step,
        "best_val_loss": best, "metrics": metrics, "args": vars(args),
        "conditioning": {"type": "single_learned_unconditional_token", "num_classes": 1, "label": 0},
    }
    ...
    protected = {path.name}
    for link_name in ("last.pt", "best.pt"):
        link = Path(checkpoint_dir) / link_name
        if link.is_symlink(): protected.add(os.readlink(link))
    candidates = sorted(p for p in Path(checkpoint_dir).glob("*.pt") if p.stem.isdigit())
...
def _load_encoder(args, device, logger):
    adapter = VQGANAdapter.from_checkpoint(args.base_config, args.base_checkpoint, str(device))
    if adapter.codebook_size != args.vocab_size:
        raise ValueError(f"codebook mismatch: tokenizer={adapter.codebook_size}, GPT={args.vocab_size}")
    for unused in ("decoder", "post_quant_conv", "loss", "transformer"):
        if hasattr(adapter.vqgan, unused): setattr(adapter.vqgan, unused, None)
    adapter.freeze(); torch.cuda.empty_cache()
    logger.info(f"Loaded frozen base tokenizer; vocabulary={adapter.codebook_size}")
    return adapter


@torch.no_grad()
def _evaluate(model, encoder, loader, device, dtype, precision, max_batches):
    ...
    for batch_index, images in enumerate(loader):
        ...
        count = tokens.numel(); totals[0] += loss.double() * count
        totals[1] += (logits.argmax(-1) == tokens).sum().double(); totals[2] += count
    dist.all_reduce(totals)
    count = max(totals[2].item(), 1)
...
def main(args):
    ...
    if args.global_batch_size < 1: raise ValueError("global batch size must be positive")
    ...
    if rank == 0:
        if output_dir.exists() and any(output_dir.iterdir()) and not args.resume:
            raise FileExistsError(f"non-empty run directory: {output_dir}")
    ...
    model = GPT_models[args.gpt_model](
        vocab_size=args.vocab_size, block_size=code_length, num_classes=1,
        cls_token_num=1, model_type="c2i", class_dropout_prob=0.0,
        resid_dropout_p=args.dropout, ffn_dropout_p=args.dropout,
        token_dropout_p=args.token_dropout, drop_path_rate=args.drop_path,
    ).to(device)
    logger.info(f"Text-free LlamaGen {args.gpt_model}: {sum(p.numel() for p in model.parameters()):,} parameters")
    logger.info("Conditioning: one learned unconditional BOS token; no captions or T5 features")
    ...
    encoder = _load_encoder(args, device, logger)
    ...
    logger.info(f"ROCOv2 train/val={len(train_set):,}/{len(val_set):,}; global batch={args.global_batch_size}; token length={code_length}")
    model = DDP(model, device_ids=[local_rank], broadcast_buffers=False)
    dtype = {"none": torch.float32, "bf16": torch.bfloat16, "fp16": torch.float16}[args.precision]
    stop = False; log_loss = 0.; log_acc = 0.; log_count = 0; last_time = time.time()
    for epoch in range(start_epoch, args.epochs):
        ...
        for images in train_loader:
            ...
            log_loss += loss.item(); log_acc += (logits.argmax(-1) == tokens).float().mean().item(); log_count += 1
            if step % args.log_every == 0:
                values = torch.tensor([log_loss, log_acc, log_count], dtype=torch.float64, device=device); dist.all_reduce(values)
                denominator = max(values[2].item(), 1); elapsed = max(time.time() - last_time, 1e-6)
                if rank == 0:
                ...
                log_loss = log_acc = 0.; log_count = 0; last_time = time.time()
            if args.max_steps and step >= args.max_steps: stop = True; break
        metrics = _evaluate(model, encoder, val_loader, device, dtype, args.precision, args.val_max_batches)
        is_best = metrics["val_loss"] < best; best = min(best, metrics["val_loss"])
        if rank == 0:
            checkpoint = _save(model, optimizer, scaler, args, checkpoint_dir, epoch, step, best, metrics, is_best)
            logger.info(f"validation step={step}: {metrics}; checkpoint={checkpoint}")
            if writer:
    ...
    if rank == 0: logger.info("Text-free LlamaGen training finished")
...
def build_parser():
    ...
    parser.add_argument("--base-config", required=True); parser.add_argument("--base-checkpoint", required=True)
    parser.add_argument("--pafr-checkpoint", required=True, help="PAFR checkpoint produced by the preceding tokenizer stage; recorded for hierarchical reproducibility.")
    parser.add_argument("--results-dir", required=True); parser.add_argument("--run-name", required=True); parser.add_argument("--resume")
    parser.add_argument("--gpt-model", choices=list(GPT_models), default="GPT-L")
    parser.add_argument("--image-size", type=int, default=256); parser.add_argument("--downsample-size", type=int, default=16); parser.add_argument("--vocab-size", type=int, default=1024)
    parser.add_argument("--epochs", type=int, default=60); parser.add_argument("--max-steps", type=int, default=0); parser.add_argument("--global-batch-size", type=int, default=64); parser.add_argument("--num-workers", type=int, default=8)
    parser.add_argument("--lr", type=float, default=1e-4); parser.add_argument("--weight-decay", type=float, default=.05); parser.add_argument("--beta1", type=float, default=.9); parser.add_argument("--beta2", type=float, default=.95); parser.add_argument("--grad-clip", type=float, default=1.)
    parser.add_argument("--dropout", type=float, default=.1); parser.add_argument("--token-dropout", type=float, default=.1); parser.add_argument("--drop-path", type=float, default=0.)
    parser.add_argument("--precision", choices=("none", "bf16", "fp16"), default="bf16"); parser.add_argument("--seed", type=int, default=23)
    parser.add_argument("--log-every", type=int, default=20); parser.add_argument("--val-max-batches", type=int, default=100); parser.add_argument("--keep-last-checkpoints", type=int, default=3)
    return parser
