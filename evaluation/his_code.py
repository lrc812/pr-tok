...
def visualize_codebook_usage_sorted_bar(usage_counts, title="Codebook Usage Sorted Bar Chart", save_dir=None):
    if isinstance(usage_counts, torch.Tensor):
        usage_counts = usage_counts.detach().cpu().numpy()
    ...
    # 按频率从高到低排序
    sorted_indices = np.argsort(usage_counts)[::-1]  # 降序索引
    sorted_frequencies = usage_counts[sorted_indices]

    plt.figure(figsize=(12, 6))
    plt.bar(range(len(sorted_frequencies)), sorted_frequencies, color="tab:blue", alpha=0.85)
    plt.title(title)
    plt.xlabel("Code Index (Sorted by Frequency Descending)")
    plt.ylabel("Usage Frequency")
    plt.grid(True, which="both", linestyle="--", linewidth=0.5, alpha=0.5)

    if save_dir:
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, "sorted_bar.jpg")
        plt.savefig(save_path, bbox_inches="tight", dpi=200)
        plt.close()
        print(f"[save] sorted bar chart -> {save_path}")
    else:
        plt.show()
...
def analyze_codebook_from_logdir(
    ...
    visualize_codebook_usage_sorted_bar(usage_counts, title="Codebook Usage Sorted Bar Chart", save_dir=save_dir)
