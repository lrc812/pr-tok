#!/usr/bin/env python3
"""Measure VQGAN codebook utilization on ImageNet validation and plot the distribution."""
...
def load_model(config_path: Path, checkpoint_path: Path, device):
...
def _text_box(draw: ImageDraw.ImageDraw, xy: tuple[int, int], text: str, font, fill):
...
def save_heatmap(counts: np.ndarray, output_dir: Path, title: str) -> Path:
    ...
    font = _load_font(24)
    small = _load_font(16)
    _text_box(draw, (40, 20), title, font, 'black')
    _text_box(draw, (40, 40 + heatmap.height), 'Dark = low usage, bright = high usage', small, '#444444')
...
def save_histogram(counts: np.ndarray, output_dir: Path, title: str) -> Path:
    ...
    font = _load_font(24)
    small = _load_font(16)
    _text_box(draw, (left, 20), title, font, 'black')
    ...
    draw.text((10, top + plot_h / 2), 'log scale not drawn', font=small, fill='#555555')
    draw.text((left, height - 50), 'Usage count per code', font=small, fill='black')
    draw.text((10, 10), 'count of codes', font=small, fill='black')
...
def main() -> None:
    ...
    parser.add_argument('--model-dir', type=str, default=str(DEFAULT_MODEL_DIR), help='Directory containing config.yaml and model.ckpt')
    parser.add_argument('--image-dir', type=str, default=None, help='ImageNet validation root. If omitted, common local defaults are tried.')
    ...
    output_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else model_dir / 'codebook_usage'
    ...
    model, config, metadata = load_model(config_path, checkpoint_path, device)
    ...
    image_size = int(config.model.params.ddconfig.resolution)
