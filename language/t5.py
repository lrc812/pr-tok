...
class T5Embedder:
    available_models = ['t5-v1_1-xxl', 't5-v1_1-xl', 'flan-t5-large']
    ...
    def __init__(self, device, dir_or_name='t5-v1_1-xxl', *, local_cache=False, cache_dir=None, hf_token=None, use_text_preprocessing=True,
        ...
        if local_cache:
            ...
            for filename in [
                ...
                hf_hub_download(repo_id='google/flan-t5-large', filename=filename, cache_dir=cache_dir,
                                force_filename=filename, token=self.hf_token)
            tokenizer_path, path = cache_dir, cache_dir
        else:
            # 假设 dir_or_name 是 HF repo id（如 google/flan-t5-large），让 Transformers 自己处理缓存
            # 不手动下载，依赖 HF hub 的缓存机制
            tokenizer_path = dir_or_name
            path = dir_or_name

        print(tokenizer_path)
        print(tokenizer_path)
        # 添加 local_files_only=True 以强制离线（如果缓存存在，会用缓存；否则报错）
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, local_files_only=True)
        self.model = T5EncoderModel.from_pretrained(path, local_files_only=True, **t5_model_kwargs).eval()
